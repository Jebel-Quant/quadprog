...
empirically on 2969 problems, where both implementations agree on every iteration
...
The result is faster than the compiled reference for $n \gtrsim 160$, by
7.1 times at $n = 700$, while remaining slower for small problems by a
factor set by interpreter dispatch rather than by arithmetic. We also report two
...
\begin{tabular}{ll}
  ...
  \midrule
  Iteration counts (added, dropped) & exact, 2969 / 2969 feasible problems \\
  Infeasibility verdict             & exact, 1031 / 1031 infeasible problems \\
  Minimiser $x$                     & max abs.\ difference 2.1e-10 \\
  Objective                         & max rel.\ difference 1.1e-12 \\
  \bottomrule
\end{tabular}
\end{center}
...
the vectorised paths dominate. The implementation carries 960 tests at
...
\begin{tabular}{r r r l}
  ...
  \midrule
  10  & 0.09  & 0.007 & 13.4$\times$ slower \\
  25  & 0.19  & 0.02  & 10.9$\times$ slower \\
  50  & 0.53  & 0.08  & 6.5$\times$ slower \\
  100 & 1.58  & 0.85  & 1.9$\times$ slower \\
  200 & 3.6   & 6.5   & \textbf{1.8$\times$ faster} \\
  400 & 14.0  & 53    & \textbf{3.8$\times$ faster} \\
  700 & 46    & 327   & \textbf{7.1$\times$ faster} \\
  \bottomrule
\end{tabular}
\end{center}

The crossover is at $n \approx 160$, located by sweeping the interval: the ratio
passes unity between $n = 150$ ($1.03\times$) and $n = 160$ ($0.94\times$).

Below the crossover the cost is interpreter dispatch: roughly 15\,$\mu$s
per iteration spread over some eighteen array operations, against
...
between 2.4 and 2.6 times \emph{slower}, with no deletions at all. The figure
bounds this application strategy rather than implicit storage as such: LAPACK
does not block \texttt{ormqr} for a single right-hand side, so it degenerates
into $k$ sequential \texttt{larf} applications, and a blocked application of the
same reflections \citep{bischof1987,schreiber1989} would change the constant
substantially.
...
The totals integrate $k$ to $n$, that is, they assume the working set grows to
fill the space, which is the least favourable case for implicit storage.
Integrating instead to a final size $K$ gives $8n^2K - 3nK^2$ against
$2n^2K + 4nK^2$, equal at $K = \tfrac{6}{7}n$; below that the flop count favours
the implicit side.
...
faster than the C extension, where the NumPy version is 13.4 times slower.
...
The resulting solver needs no compiler and is, for $n \gtrsim 160$, several times
...
\citep{quadprogpy} run in continuous integration on CPython 3.12, 3.13 and 3.14.
