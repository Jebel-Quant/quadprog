...
the crossover to $n \approx 65$ and the margin to 68 times at $n = 1600$. We also report two
negative results in full: leaving the orthogonal factor implicit in the manner of
LAPACK's \texttt{geqrf}/\texttt{ormqr} is 2.4 to 2.6 times
\emph{slower}, and a Numba \citep{numba2015} port wins by an order of magnitude at
$n = 10$ but loses by 2.5 times at $n = 700$.
\end{abstract}
...
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
  ...
  \item a differential validation methodology which compares not merely
        ...
        (\S\ref{sec:agreement});
  \item two negative results, reported with the measurements that killed them
        (\S\ref{sec:negative}).
\end{itemize}
...
iterations this is the difference between $O(n^3)$ and $O(n^4)$, and we quantify
it in \S\ref{sec:negative}.
...
a library least-squares routine should simply be called here; \S\ref{sec:negative}
shows why that costs a factor of 57.
\end{remark}
...
\texttt{gemv} regardless of $k$. Section~\ref{sec:negative} shows what happens
when this folding is undone.
\end{remark}
...
This is a floor set by the language implementation, not by the algorithm, and
\S\ref{sec:negative} confirms it by removing it.
...
The first factor yields to a different method, and not a new one. A primal-dual
active set guesses the whole active set $\mathcal{A}$ at once, solves the
equality-constrained problem it induces,
\[
...
Read as a primal-dual active-set strategy this is the semismooth Newton method of
\citet{hintermuller2002}; read as a rule for exchanging several indices at once it
is block principal pivoting on the linear complementarity problem
\citep{judice1994,murty1988,cottle1992}. What is specific
here is the constraint class, and the constraint class is what decides how much
can be guaranteed.

\paragraph{Why no finite-termination theorem is available.}
For the bound-constrained program $\min \tfrac12 x\T A x - b\T x$ subject to
$x \ge 0$, the system solved on a candidate free set $\mathcal{F}$ is
$A_\mathcal{F} x_\mathcal{F} = b_\mathcal{F}$, a \emph{principal submatrix} of
$A$. When $A \succ 0$ every principal submatrix is again positive definite, so $A$
is a $P$-matrix: every principal pivot is well defined, and guarding the block
exchange with a least-index rule in the manner of Bland removes what degeneracy is
left, giving finite termination at the unique minimiser
\citep{judice1994,murty1988}. That guarantee even survives inexact inner solves,
once their residual is small against the problem's decision margin
\citep{schmelzer2026}.

None of it is available here. The general constraints $C\T x \ge b$ induce
$C_\mathcal{A}\T G^{-1} C_\mathcal{A}$, which is positive definite exactly when
$C_\mathcal{A}$ has full column rank --- a property of the \emph{guess}, not of
the data. A guessed active set may hold linearly dependent constraints, and then
the pivot is undefined rather than merely awkward. There is no $P$-matrix to
appeal to and no finite-termination theorem to inherit, which is why the guard
below is a certificate and a fallback rather than an anti-cycling rule. The
Cholesky of the working-set system doubles as the rank test: a dependent guess
fails there instead of returning something plausible.

\paragraph{The certificate.}
Since the method may therefore fail to converge, the trade is sound only because
the answer is checked. The program is strictly convex, so the KKT conditions are
sufficient and not merely necessary: a point satisfying them is \emph{the}
minimiser. Every candidate is certified before it is returned, and one that fails
is discarded in favour of the exact walk. This is not a formality. Over 1164
converged attempts, two had settled on a set that was not optimal --- one of them
$0.85$ from the true minimiser --- and the certificate rejected both; among the
points it accepted, the largest disagreement with the exact walk was
$3.3\times10^{-15}$.

Nor is the rank-deficient case hypothetical, and it is not even stable across
machines. An instance whose guessed set was reliably singular under one vendor's
BLAS converged under another's, the two libraries disagreeing about the
conditioning of the same matrix; the discrepancy first surfaced as a test that
passed on one platform and failed on every other. Whether the certified path is
taken is thus partly a property of the arithmetic underneath it, which is a
further reason to check the answer rather than assume it.
...
Both of the following were pursued to working prototypes, verified against the
shipped implementation, and measured. We report them because the reasoning that
motivated each is more persuasive than the outcome, and because negative results
of this kind are ordinarily discarded.

\subsection{Leaving the orthogonal factor implicit}

Insertion updates $J$ explicitly, at $O(n(n-k))$, and this is now half the
runtime. LAPACK's \texttt{geqrf}/\texttt{ormqr} pair suggests an alternative:
store the Householder vectors and never form $Q$
\citep{bischof1987,schreiber1989}. Insertion then costs essentially nothing,
because $d$ of \eqref{eq:d} \emph{already is} the reduced column --- the
reflection is read off it and appended, and no matrix is touched. The
factorisation is maintained as $J_0 = R_G^{-1}$ together with a chain
$H_1 \cdots H_k$, and \eqref{eq:d} and \eqref{eq:z} become a triangular product
followed by an \texttt{ormqr} application.

The prototype reproduces the shipped implementation exactly --- identical
iteration counts on 300 problems, worst $|\Delta x| = 4.7e-12$ --- and is
between 2.4 and 2.6 times \emph{slower}, with no deletions at all:
...
\begin{tabular}{lll}
  ...
  \midrule
  $d$      & one \texttt{gemv}, $2n^2$ & \texttt{trmv} $n^2$ + \texttt{ormqr} $\sim\!4nk$ \\
  $z$      & \texttt{gemv}, $2n(n-k)$  & \texttt{ormqr} $\sim\!4nk$ + \texttt{trmv} $n^2$ \\
  insert   & $4n(n-k)$                 & free \\
  \midrule
...
implicitly stored $Q$ costs $O(nk)$, and the solver applies it \emph{twice per
iteration} --- precisely the work the explicit update performs \emph{once}.
Forming $J$ amortises the accumulated $Q$ into a single dense matrix so that
every subsequent application is one \texttt{gemv} irrespective of $k$; that is
the entire reason to form it. Implicit storage is advantageous when $Q$ is
applied rarely relative to the number of reflections, as in a one-shot
factorisation with few right-hand sides. Here it is applied twice per reflection,
which is its worst case.

Deletion supplies a second, independent objection. A Givens chase cannot be
absorbed into a stored Householder chain, so a deletion becomes a
refactorisation: 82\% of runtime on a problem with 200 of them, and
2.48 times slower overall. We measured deletion frequency across problem
classes --- 0\% of steps for box and budget-plus-bounds problems,
2.2\% for random dense $C$, 45\% for an adversarial
case constructed to force churn --- and concluded that a hybrid would have been
viable had the insertion side won. It does not.

\begin{remark}[Library routines]
SciPy ships compiled routines for precisely this updating problem, namely
\texttt{qr\_insert}, \texttt{qr\_delete} and \texttt{qr\_update}. Over
the full sequence of 383 column insertions at $n = 700$ they take 39.5\,ms,
against 31.3\,ms here. The difference is that the library API
returns fresh arrays, and so reallocates a 3.9\,MB $Q$ on every call. Solving \eqref{eq:lsq} from scratch each
iteration with \texttt{lstsq} costs 11.5\,ms at $k = 383$ against
0.5\,ms for the update --- which, over the run, is the
$O(n^4)$ versus $O(n^3)$ distinction of \S\ref{sec:method}, and amounts to a
factor of roughly 57.
\end{remark}

\subsection{Compiling with Numba}

The small-$n$ deficit of \S\ref{sec:perf} is dispatch overhead, and a
just-in-time compiler removes dispatch. We ported the solver to Numba
\citep{numba2015}, giving it the best available shot: both matrix--vector
products are expressed so that their arguments remain contiguous, which is what
allows Numba to route them to BLAS \texttt{gemv} exactly as the NumPy version
does, and compilation is cached so that JIT time is excluded.

The port is faithful --- identical iteration counts on 300 problems, worst
$|\Delta x| = 8.6e-12$.

\begin{center}
\begin{tabular}{r r r l r r l}
  \toprule
  & \multicolumn{3}{c}Dense $C$ (ms) & \multicolumn{3}{c}Box (ms) \\
...
At $n = 10$ the compiled version is between 1.04 and 1.33 times
faster than the C extension, where the NumPy version is 12.5 times slower.
The interpreter floor is therefore not a property of the algorithm. Above the
crossover --- about $n = 50$ dense, $n = 250$ box --- Numba loses, because the
rank-one update becomes LLVM-generated loops where the NumPy version calls a
vendor \texttt{ger}, and that update is half of large-$n$ runtime.

We document rather than adopt this. Numba's \texttt{llvmlite} is a
38\,MB dependency which pins NumPy backwards; it contradicts the
package's central claim of needing no compiler; it requires a second copy of a
numerically delicate loop, kept differentially tested against the first; and the
sizes at which it wins are those where every implementation is already fast in
absolute terms, 0.08\,ms against 0.006\,ms.
